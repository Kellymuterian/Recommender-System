...
@st.cache_data
def load_data():
    return pd.read_csv("processed_df.zip")
...
def search_products(query, top_n=5, store=None, pricing=None):
...
if st.button("Search"):
    search_products(query=search_query, top_n=top_n, store=store)
...
with open(csv_file_path, "w", newline="") as file:
    ...
    for query, recommendations in search_history:
        for index, row in recommendations.iterrows():
            writer.writerow([query, row['PRODUCT_NAME'], row['PRODUCT_PRICE'], row['PRODUCT_BRAND']])
